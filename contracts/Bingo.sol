...
contract Bingo is Ownable {
    ...
    function join() public {
        ...
        playerBoard[msg.sender] = encodeBoolMatrix(_marked, playerBoard[msg.sender]);
        playersJoined++;
    }
     ...
     */
    function getMarkedSquares(address player) external view returns(bool[][] memory _marked) {
    ...
    function getBoardNumber(address player, uint8 row, uint8 col) external view returns (uint8) {
     ...
     */
    function getBoardNumberOptimized(address player, uint row, uint col) public view returns(uint8 boardNumber) {
     ...
     * Grid must be 5x5
     */
    function encodeBoolMatrix(bool[][] memory matrix, uint256 startNum) public pure returns(uint256 result) {
        uint shift = 208;
        for(uint i = 0 ; i < 5 ; ++i) {
            for(uint j = 0 ; j < 5 ; ++j) {
                // Store the bool value starting from bit 208 in the uint256
                if(matrix[i][j])
                    startNum |= uint(1 << shift);
                shift += 1;
            }
        }
        return startNum;
     ...
     * Uses the same uint256 which stores the grid
     */
    function decodeBoolMatrix(uint256 matrixNum) public pure returns(bool[][] memory matrix) {
        uint shift = 208;
        matrix = new bool[][](5);
        for(uint i = 0 ; i < 5 ; ++i) {
            matrix[i] = new bool[](5);
     ...
     * Grid must be 5x5
     */
    function encodeGrid(uint8[][] memory grid, uint256 startNum) public pure returns(uint256 result) {
        uint shift = 0;
        for(uint i = 0 ; i < 5 ; ++i) {
            for(uint j = 0 ; j < 5 ; ++j) {
